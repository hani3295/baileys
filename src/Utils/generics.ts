	...
	ev.on('connection.update', async({ qr }) => {
		if(qr) {
			if (qrcode) {
				...
				console.log(`[WARNING]: the module qrcode-terminal is not installed. Please install it to get a QR code.\nuse 'npm install qrcode-terminal' or 'yarn add qrcode-terminal' to install.`)
			}
		}
	})
}
